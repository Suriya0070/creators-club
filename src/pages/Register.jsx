...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (form.phone && !/^[6-9]\d{9}$/.test(form.phone.replace(/\D/g, ''))) e.phone = 'Enter a valid 10-digit Indian phone number'
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`input-field pl-10 ${extra || ''} ${errors[name] ? 'border-red-500/60' : ''}`}
      ...
      <div className="relative w-full max-w-lg px-4 py-20">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Field name="name" label="Full Name *" placeholder="Aryan Sharma" icon={User} />
            <Field name="email" label="Email *" type="email" placeholder="you@example.com" icon={Mail} />
            <Field name="phone" label="Phone (optional)" type="tel" placeholder="+91 98765 43210" icon={Phone} />
            <Field name="password" label="Password *" type={showPass ? 'text' : 'password'} placeholder="Min. 8 characters" icon={Lock} extra="pr-10" />
            <Field name="confirm" label="Confirm Password *" type={showPass ? 'text' : 'password'} placeholder="Re-enter password" icon={Lock} />
            ...
            >
              {loading ? (
                <><div className="w-4 h-4 border-2 border-navy-900/40 border-t-navy-900 rounded-full animate-spin" /> Creating account...</>
              ) : (
